...
DINO_0=pygame.image.load(os.path.join("Assets/Dino", "none.png"))    #style của dino
...
class Dinosaur:
    ...
    def __init__(self):             #khởi tạo khủng long bất cứ khi nào khủng long được tạo lại
        ...
        self.jump_vel = self.JUMP_VEL   #khởi tạo thuộc tính jump_vel bằng 1 hằng số JUMP_VEL mặc định
    ...
    def update(self, userInput):                      #hàm cập nhật khủng long mỗi lần lặp lại vòng while
        ...
        if (userInput[pygame.K_UP] or userInput[pygame.K_SPACE]) and not self.dino_jump:     #khi bấm up hoặc Space mà khủng long không jump thì khủng long jump
            self.dino_duck = False
            self.dino_run = False
            self.dino_jump = True
            ...
            self.dino_run = False
            self.dino_jump = False
        elif not (self.dino_jump or userInput[pygame.K_DOWN]):  #nếu không jump hay duck thì run
            self.dino_duck = False
            self.dino_run = True
            self.dino_jump = False
        if userInput[pygame.K_0]: self.dino_style = DINO_0
        if userInput[pygame.K_1]: self.dino_style = DINO_1  # Nếu người chơi bấm phím 1 khủng long sẽ thay đổi style thành DINO1
        if userInput[pygame.K_2]: self.dino_style = DINO_2  # Nếu người chơi bấm phím 2 khủng long sẽ thay đổi style thành DINO2
        if userInput[pygame.K_3]: self.dino_style = DINO_3  # Nếu người chơi bấm phím 3 khủng long sẽ thay đổi style thành DINO3
        if userInput[pygame.K_4]: self.dino_style = DINO_4  # Nếu người chơi bấm phím 4 khủng long sẽ thay đổi style thành DINO4
        if userInput[pygame.K_5]: self.dino_style = DINO_5  # Nếu người chơi bấm phím 5 khủng long sẽ thay đổi style thành DINO5    
        if userInput[pygame.K_6]: self.dino_style = DINO_6  # Nếu người chơi bấm phím 6 khủng long sẽ thay đổi style thành DINO6
        if userInput[pygame.K_7]: self.dino_style = DINO_7  # Nếu người chơi bấm phím 7 khủng long sẽ thay đổi style thành DINO7
        if userInput[pygame.K_8]: self.dino_style = DINO_8  # Nếu người chơi bấm phím 8 khủng long sẽ thay đổi style thành DINO8
    ...
    def jump(self):
        ...
        if self.dino_jump:                      #nếu trạng thái nhảy của khủng long được kích hoạt
            self.dino_rect.y -= self.jump_vel * 4 #tọa độ y của khủng long sẽ thay đổi dựa vào chỉ số jump_vel
    ...
    def draw(self, SCREEN):                                    #hàm vẽ lên màn hình
        SCREEN.blit(self.image, (self.dino_rect.x, self.dino_rect.y))  #Vẽ hình ảnh của khủng long lên màn hình game
        if(self.dino_style == DINO_1):
            SCREEN.blit(self.dino_style, (self.dino_rect.x-30, self.dino_rect.y-180)) #dino 1
...
class Cloud:                                       #vẽ mây
    def __init__(self):
        self.x = SCREEN_WIDTH + random.randint(800, 1000)  #vị trí x của mây = độ rộng của màn hình + random(800 -> 1000)
        self.y = random.randint(50, 100)                #vịt trí y của mây = random(50 -> 100)
...
def main():
    ...
    def background():
        ...
        SCREEN.blit(BG, (image_width + x_pos_bg, y_pos_bg)) #vẽ nối thêm 1 backgroud nữa 
    ...
    while run:
        ...
        player.update(userInput)          #cập nhật khủng long với mỗi sự kiện tương tác của người chơi
...
def menu(death_count):
    ...
    while run:
        ...
        if death_count == 0:        #số lần chết = 0 thì bấm để bắt đầu
            ...
            scoreRect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 50)  #vị trí của score
            SCREEN.blit(score, scoreRect) #vẽ score lên màn hình
        textRect = text.get_rect() 
        textRect.center = (SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2) #vị trí của text
